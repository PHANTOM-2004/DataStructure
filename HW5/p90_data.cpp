...
public:
  ...
  struct NODE {
     ...
     */
    void UpdateHeight() {
      height_ = std::max((left_ ? left_->height_ : 0),
                         (right_ ? right_->height_ : 0)) +
                1;
    }
    /**
     * @name    bool Find(int elem, NODE *&last, NODE *&thiss, NODE *&next, bool
     * &found)
     ...
     * @param   found
     * 是否已经找到，true表示已经找到，接下来递归找到后继节点就退出
     * @return  是否找到后继，查找结束
     */
      ...
      if (index % 2 == 0) { // 一般的数据会考察树的性能，即输入数据是有序的
        if (i == 0) {
          ...
          last_insert = elem;
                   ...
                   0.7 * n) { // 前70%操作中绝大部分为插入，且插入数据基本有序
          int select = rand() % 20;
          if (select < 16) {
            ope = 1;
            if (index % 4 == 0) {
              elem = std::max(1, last_insert -
                                     (rand() % (12 * SORTTREE::RANDOM_GAP) -
                                      SORTTREE::RANDOM_GAP));
              last_insert = std::min(last_insert, elem);
            } else {
              elem = last_insert + (rand() % (12 * SORTTREE::RANDOM_GAP) -
                                    SORTTREE::RANDOM_GAP);
          ...
          if (select < 2) {
            ...
            if (index % 4 == 0) {
              ...
              elem =
                  SORTTREE::MAX_ELEM - rand() % int(SORTTREE::MAX_ELEM * 0.2);
            }
          } else if (select == 14) {
            ope = 4;
          } else {
